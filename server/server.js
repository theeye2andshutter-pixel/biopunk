...
// Inicializar OpenAI con la clave protegida de las variables de entorno
const openai = new OpenAI({
  apiKey: process.env.OPENAI_API_KEY,
});

// El SYSTEM PROMPT con el lore de tu juego
...
## EJEMPLO:
Usuario: "¿Dónde me escondo de BIOMODS?"
Respuesta: "> Huye a PTLLCT, OXIDO_VIVO. El micelio negro te ocultará de sus satélites. No hay tiempo."`;

// Ruta (endpoint) a la que llamará tu frontend
  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: LORE_SYSTEM_PROMPT,
        },
        {
          role: "user",
          content: `Alias del operador: [${userAlias}]. Mensaje transmitido: "${userMessage}"`,
        },
      ],
    });
    ...
    res.status(500).json({
      reply: '> ERR_SYS_500: Interferencia corporativa. Enlace neural inestable.',
    });
  }
});
...
app.get('/health', (req, res) => {
  res.json({ status: 'Nodo activo' });
});

const PORT = process.env.PORT || 5000;
app.listen(PORT, () => {
  console.log(`🔴 Nodo de Red Neural activo en puerto ${PORT}`);
  console.log(`📡 Sector Cusco 2099 — Zona Sombra: ONLINE`);
});
